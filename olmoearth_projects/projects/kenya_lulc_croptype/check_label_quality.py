...
if __name__ == "__main__":
    ...
    windows = dataset.load_windows(show_progress=True, groups=["kenya_cropland_maize"])
    labels, geometry = [], []
    for window in windows:
        split = window.options["split"]
        if split in splits_to_keep:
            if args.crop_type:
                labels.append(window.options["maize_category"])
            else:
                labels.append(window.options["lulc_category"])
